...
function* expandBraces(str) {
    ...
    function parse(str) {
        ...
        let pos = 0;
        while (str[pos])
            if (str[pos] !== OPEN_BR) items = combine(items, [readUntil([OPEN_BR])]);
            else 
            {
                pos += 1;
        ...
        function parseExpr() {
            ...
            while (str[pos] !== CLOSE_BR) 
                ...
                else items = items.concat(parseExprPart());
        ...
        function parseExprPart() {
            ...
            while (str[pos] !== SEPARATOR && str[pos] !== CLOSE_BR) 
                if (str[pos] !== OPEN_BR) items = combine(items, [readUntil([SEPARATOR, OPEN_BR, CLOSE_BR])]);
                else 
                {
                    pos += 1;
                    items = combine(items, parseExpr());
...
function canDominoesMakeRow(dominoes) {
    ...
    while (dominoes.length != 0)
	{
        ...
        i++;
        if (i == dominoes.length) return false;
    }
    return true;
}
